...
class Menu(object):
    ...
    def process_models(self, app):
        ...
        for model_def in models_def:
            if 'models' in model_def:
                models.append(self.make_app(model_def))
            else:
    ...
    def activate_models(self, app, match_by_name=False, level=1):
        for model in app['models']:
            if 'models' in model:
                subapp = model

                # Make 'model' key as 'models' to unite activation logic
                if 'model' in subapp and not subapp['models']:
                    subapp['models'] = [subapp['model']]
            ...
            if model['is_active']:
                ...
                if not self.app_activated and level == 1:
