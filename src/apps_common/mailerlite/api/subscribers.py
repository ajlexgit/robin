...
def create(group_id, email, first_name='', last_name='', company='', status='unconfirmed', resubscribe=True):
    """ Создание подписчика """
    fields = {
        'name': first_name,
        'last_name': last_name,
    ...
    return request('groups/%d/subscribers' % group_id, method='POST', data={
        'email': email,
